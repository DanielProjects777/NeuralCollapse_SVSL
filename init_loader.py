...
def init(dataset_conf, use_consistency_loss: bool = False, hyper_parameters: dict = None):
    ...
    model = eval(
        f"models.{dataset_conf['model_conf']['model_name']}(pretrained=False, num_classes={dataset_conf['C']})")

    root_logger.info("after model eval")

    layers = init_layers(model, hyper_parameters)
    ...
    trainer = Trainer(conf=dataset_conf, model=model, optimizer=optimizer, criterion=criterion,
                      ...
                      layers=layers, distributed=distributed, hyper_parameters=hyper_parameters,
